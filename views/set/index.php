        ...
        <section class="categories main-content">
            ...
            <div class="wrapper">
                <div class="categories-wrapper">
                    ...
                    <div class="filter" name="<?= $filters[0]['filters_category'] ?>">
                        <ul class="filter-list">
                            <?php foreach ($filters as $key => $value): ?>
                                <li class="filter-item">
                                <span class="filter-text">
    <?= Html::encode($value['filters_name']) ?> 
                                </span>
                                </li>
                    ...
                    <div id="whats_cat" class="categories-row row" namer="<?= Html::encode($smak) ?>">
                        <?php $count = 0 ?>
                        <?php foreach ($product as $key => $value): ?>
                            <div class="categories-col col-4">
                                <div class="categories-item">
                                    <a href="/set/<?= Html::encode($smak) ?>/<?= Html::encode($value['product_alias']) ?> "
                                        ...
                                        <div class="clearfix">
                                            <div class="left">
                                                ...
                                                <div class="categories-size">
                                                    <div><?php if (strlen($value['product_length']) > 0) { ?>
                                                            <?= Html::encode($value['product_length']) ?>см,
                                                        <?php } ?>
                                                        <?= Html::encode($value['product_weight']) ?>гр.
                                                    </div>
                                                    <div><?= Html::encode($value['product_kkal']) ?>ккал/100гр.</div>
                                                </div>
                                            </div>
                                            <a href="javascript:void(0)" class="categories__btn-order right mt10"
                                               ider="<?= Html::encode($value['product_id']) ?>" price="product_price1"
                                               weight="product_weight" kkal="product_kkal" tokens="product_balls">
                                                Хочу!
                                            </a>
                                        </div>
                                        <?php if ($value['product_price2'] != 0) { ?>
                                            ...
                                            <div class="clearfix">
                                                <div class="left">
                                                    <!--                                                <div class="categories-price">-->
                                                    <!--                                                    <strong>-->
                                                    <? //= Html::encode($value['product_price2']) ?><!-- </strong>-->
                                                    <!--                                                </div>-->
                                                    <?php if ($value['product_choice'] == 1) { ?>
                                                        <div class="categories-price">
                                                            <strong> <?= Html::encode($endoprice / 2) ?></strong>
                                                            <sup class="h6 text-gray">
                                                                <?= Html::encode($value['product_price2']) ?>
                                                            </sup>
                                                        </div>
                                                    <?php } else if ($value['product_choice'] == 2) { ?>
                                                        <div class="categories-price">
                                                            <strong> <?= Html::encode($value['product_fixprice'] / 2) ?></strong>
                                                            <sup class="h6 text-gray">
                                                                <?= Html::encode($value['product_price2']) ?>
                                                            </sup>
                                                        </div>
                                                    ...
                                                    <div class="categories-size">
                                                        <div>
                                                            <?php if (strlen($value['product_length2']) > 0) { ?>
                                                                <?= Html::encode($value['product_length2']) ?>см,
                                                            <?php } ?>
                                                            <?= Html::encode($value['product_weight_2']) ?>гр.
                                                        </div>
                                                        <div><?= Html::encode($value['product_kkal2']) ?>ккал/100гр.
                                                        </div>
                                                    </div>
                                                </div>
                                                <a href="javascript:void(0)" class="categories__btn-order right mt10"
                                                   ider="<?= Html::encode($value['product_id']) ?>"
                                                   price="product_price2" weight="product_weight_2" kkal="product_kkal2"
                                                   tokens="product_balls2">
                                                    Хочу!
                                                </a>
                                            </div>
                                        <?php } ?>
